"""Variant v8 — FoundryAgent + Local MCP (FunctionTool wrappers) + App Insights.

Goal of this variant
--------------------
Empirically answer: "Was the v7 improvement due to ``FoundryChatClient`` per
se, or was it due to (a) no shared service-side conversation and (b) Local
MCP?". v8 keeps everything from v7 EXCEPT it swaps the agent abstraction:

- v7:  ``Agent(client=FoundryChatClient(...), tools=[mcp_tool])``
- v8:  ``FoundryAgent(agent_name=..., agent_version=..., tools=mcp_tool.functions, allow_preview=True)``

Rationale
~~~~~~~~~
- The agents remain visible in Foundry Studio under their PromptAgent names.
- The agent definitions are pure *shells* (no instructions, no tools, no
  json_schema). See ``update_agents_v8.py``. Everything is supplied
  client-side at runtime via inline ``{system, user}`` messages and
  ``options={"response_format": ...}`` — identical to v7.
- ``allow_preview=True`` is REQUIRED. Without it the FoundryAgent code path
  strips ``tools`` from the request body (see ``agent_framework_foundry/_agent.py``
  lines ~366-373: "Strip tools from request body — Foundry API rejects requests
  with both agent endpoint and tools present"). With preview enabled, our
  client-side FunctionTools are sent alongside ``agent_reference`` and the
  function invocation layer dispatches calls locally → ``execute_tool`` spans.
- We attach ``MCPStreamableHTTPTool.functions`` (a list of FunctionTool
  instances built by MAF from the MCP server's ``tools/list``). This gives
  the model the same listTables / getSchema / executeQuery signatures it had
  in v7, but dispatched via the FunctionTool layer rather than the MCPTool
  layer. ``execute_tool <name>`` spans are emitted in both cases.

Pre-requisites
--------------
- Run ``update_agents_v8.py`` once before running this script to publish the
  shell PromptAgent versions in Foundry. The script auto-picks the latest
  version of each agent.

Everything else (audit provider, 3 executors, force_flush, observability
bootstrap) is identical to v7.
"""
...
# Wire telemetry to Azure Application Insights — same as v7.
...
# ---------------------------------------------------------------------------
# Instructions — same templates as v7 (rendered inline as system message)
# ---------------------------------------------------------------------------
...
4. For DataQuery, call the listTables tool first and populate candidate_tables only with tables it returns. Never invent table names.
...
- Use ONLY the structured inputs supplied here and the metadata returned by tools.
...
3. For each candidate table, call the getSchema tool to retrieve column definitions and join hints before generating SQL. Do not assume schema.
...
Mission:
- Execute the SQL query from the plan using the executeQuery tool.
...
# ---------------------------------------------------------------------------
# Audit-only HistoryProvider (identical to v7)
# ---------------------------------------------------------------------------
...
def _build_user_message(user_text: str) -> list[Message]:
    # NOTE: ``FoundryAgent._prepare_messages_for_azure_ai`` strips system/
    # developer messages from the request (and the extracted text is
    # discarded, not forwarded as instructions). We therefore inject
    # per-call instructions via ``options={"instructions": ...}`` (see
    # ``ChatAgent._merge_options`` which concatenates default+per-call
    # instructions). Sending only the user message keeps the request clean.
...
def _latest_version(client: AIProjectClient, agent_name: str) -> str:
    """Return the highest version string of the given PromptAgent."""
    versions = list(client.agents.list_versions(agent_name=agent_name))
    if not versions:
        raise RuntimeError(
            f"No versions found for agent '{agent_name}'. "
            "Run `python update_agents_v8.py` first to publish shell versions."
        )
    # list_versions returns descending; first item is the latest. Use the
    # ``version`` attribute (string), which is what FoundryAgent expects.
    return str(versions[0].version)


# ---------------------------------------------------------------------------
# Custom executors — same structure as v7 but typed against FoundryAgent
# ---------------------------------------------------------------------------
...
class IntentStep(Executor):
    ...
    async def run(
        ...
        messages = _build_user_message(user_question)
        response = await self._agent.run(
            messages,
            options={
                "instructions": INTENT_INSTRUCTIONS_TPL,
...
class SqlBuilderStep(Executor):
    ...
    async def run(
        ...
        messages = _build_user_message(bundle.user_question)
        response = await self._agent.run(
            messages,
            options={
...
async def main() -> None:
    ...
    # Resolve latest version of each shell PromptAgent via a sync client.
    # ``list_versions`` is a one-off lookup; no need for the async credential.
    with SyncDefaultAzureCredential() as sync_credential, AIProjectClient(
        endpoint=project_endpoint, credential=sync_credential
    ) as project_client:
        versions = {
            key: _latest_version(project_client, name)
            for key, name in AGENT_NAMES.items()
        }
    print(
        "Using PromptAgent versions: "
    ...
    async with (
        ...
        # --- Three FoundryAgents (one per role) -------------------------------
        # ``allow_preview=True`` is REQUIRED for client-side FunctionTools to be
        # sent in the request body (see module docstring).
        # ``.functions`` returns the list of FunctionTool wrappers MAF built
        # from the MCP server's tools/list response.
        def _make_agent(role_key: str, tool: MCPStreamableHTTPTool) -> FoundryAgent:
            return FoundryAgent(
                project_endpoint=project_endpoint,
                agent_name=AGENT_NAMES[role_key],
                agent_version=versions[role_key],
        ...
        intent_agent = _make_agent("intent", mcp_list)
        sql_builder_agent = _make_agent("sql", mcp_schema)
        query_executor_agent = _make_agent("executor", mcp_exec)
        ...
        _print_usage_report(
            usage,
            label="variant v8 — FoundryAgent + Local MCP (FunctionTool wrappers)",
        )
        _print_audit_log(audit)

    # Force-flush OTel exporters
